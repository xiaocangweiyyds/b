...
public class RandomServlet extends HttpServletBean {

	private static final long serialVersionUID = 1L;
	private final static Random random = new Random();
	// ���������ʽ
	private final static int[] fontStyle = { Font.HANGING_BASELINE, Font.ITALIC, Font.LAYOUT_LEFT_TO_RIGHT,
			Font.LAYOUT_NO_LIMIT_CONTEXT, Font.LAYOUT_NO_START_CONTEXT, Font.LAYOUT_RIGHT_TO_LEFT, Font.CENTER_BASELINE,
			Font.PLAIN, Font.ROMAN_BASELINE, Font.TRUETYPE_FONT, Font.TYPE1_FONT, Font.BOLD };
	...
	public void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		session.setAttribute("rand", value); // ��֤�����ɺú�,�ŵ�session�д�����,�����¼ʱ�ó����Ա�
		g.dispose();
		ImageIO.write(image, "JPEG", response.getOutputStream()); // ��ͼƬд��ҳ�� (image��ͼƬ����������)
	}

	private static Font getFont() {
		return new Font(font[(int) (Math.random() * 4)], fontStyle[(int) (Math.random() * 12)],
				font_size[(int) (Math.random() * 10)]);
	}

	private static Color randColor(int fc, int bc) {// ������Χ��������ɫ
		if (fc > 255) {
			fc = 255;
		}
		if (bc > 255) {
	...
	private static char[] c = { 'a', 'b', 'c', 'd', 'e', 'f', 'g', 'h', 'j', 'k', 'm', 'n', 'p', 'q', 'r', 's', 't',
			'u', 'v', 'w', 'x', 'y', 'z', 'A', 'B', 'C', 'D', 'E', 'F', 'G', 'H', 'J', 'K', 'L', 'M', 'N', 'P', 'Q',
			'R', 'S', 'T', 'U', 'V', 'W', 'X', 'Y', 'Z', '1', '3', '5', '6', '7', '8', '9', '0' };
